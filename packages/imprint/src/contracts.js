...
function evaluateCrank(input) {
    ...
    if (exampleCount >= 1)
        cran = 4;
    if (exampleCount >= 2 && hasRichConnector)
        cran = 5;
...
function bloomFromText(text) {
    if (/\b(analyze|compare|contrast|distinguish|examine)\b/.test(text))
        return 'analyze';
    if (/\b(apply|use|demonstrate|implement|solve)\b/.test(text))
        return 'apply';
    if (/\b(evaluate|judge|defend|critique|recommend)\b/.test(text))
        return 'evaluate';
    if (/\b(synthesize|create|design|construct|propose)\b/.test(text))
        return 'synthesize';
    if (/\b(understand|explain|describe|summarize|interpret)\b/.test(text))
        return 'understand';
    if (/\b(remember|list|name|define|recall|identify)\b/.test(text))
        return 'remember';
    return undefined;
}
