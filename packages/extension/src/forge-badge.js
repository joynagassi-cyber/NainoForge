...
exports.injectForgeBadge = injectForgeBadge;
exports.injectYouTubeBadge = injectYouTubeBadge;
const readability_js_1 = require("./extract/readability.js");
...
function injectForgeBadge(doc) {
    if (doc.getElementById(BADGE_ID))
        return;
    injectStyle();
    ...
    const btn = createButton('Forger cette page', async () => {
        ...
        try {
            const result = (0, readability_js_1.extractArticle)(doc, location.href);
            ...
            if (content)
                content.textContent = (content.getAttribute('data-original') ?? 'Forger cette page');
...
function injectYouTubeBadge(doc) {
    if (doc.getElementById(BADGE_ID))
        return;
    injectStyle();
    const root = doc.createElement('div');
    ...
    const btn = createButton('Forger cette vidéo', async () => {
        ...
        try {
            ...
            btn.removeChild(btn.firstElementChild);
            if (content)
                content.textContent = (content.getAttribute('data-original') ?? 'Forger cette vidéo');
            btn.classList.remove('nf-forge-btn--loading');
            btn.disabled = false;
            btn.removeAttribute('aria-busy');
        }
...
//# sourceMappingURL=forge-badge.js.map