  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        <div className="mb-6">
          <h3 className="text-h3 font-semibold text-text-primary mb-3">Vos concepts et maîtrise</h3>
          ...
          renderMessage={({ message, index }) => (
            <div
              ...
              className={cn(
                ...
                message.role === "user" ? "ml-auto bg-primary text-surface-base" : "mr-auto bg-surfaceSecondary text-foreground"
              )}
            >
              <p>{message.content}</p>
              {message.data?.confidence !== undefined && (
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-caption text-foregroundMuted">Confiance évaluée</span>
                  <span className="text-caption font-medium text-accent-warm">{message.data.confidence}%</span>
                </div>
              )}
            </div>
          )}
            ...
            <div className="border-t border-border px-4 py-3 mt-6">
              <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="flex gap-2">
                <input
                  ...
                  disabled={disabled}
                />
                <Button type="submit" variant="primary" size="icon" disabled={!input.trim() || submitting || disabled} iconLeft={<Send className="h-4 w-4" />} />
              </form>
            </div>
          )}
          renderLoading={() => <div className="mr-auto mt-2"><Spinner size="sm" label="Réflexion..." /></div>}
        />
      </div>
    </div>
  );
